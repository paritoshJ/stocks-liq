...
const RecievableScreen = props => {
  ...
  const renderEmptyPage = () => {
    return (
      <EmptyPageView
        icon={renderSvgIcon}
        title={I18n.t('noItemTitle', {tabName: I18n.t('receivable_menu')})}
        message={I18n.t('noItemAddText', {tabName: I18n.t('receivable_menu')})}
        buttonTitle={`+ ${I18n.t('receivable_menu')}`}
        onAddClick={() => {
          props.navigation.navigate('AddRecievebaleScreen', {
            onAddBhejanPress: onAddBhejanPress,
          });
        }}
      />
    );
  ...
  const renderAddButtom = () => {
    return (
      <TouchableOpacity
        onPress={() => {
          props.navigation.navigate('AddBhejanScreen', {
            onAddBhejanPress: onAddBhejanPress,
          });
        }}
